...
DWORD CSoundBuffer::GetPosition(){

	DWORD position;

	if (m_pDSBuffer->GetCurrentPosition(&position,NULL) != S_OK) {
		::OutputDebugString(TEXT("*** Error - 再生位置取得失敗(CSoundBuffer_GetPosition)\n"));
		return 0;
	}
...
DWORD CSoundBuffer::GetSize(){

	DSBCAPS dsbcaps;
	dsbcaps.dwSize = sizeof(dsbcaps);

	if (m_pDSBuffer->GetCaps(&dsbcaps) != S_OK) {
		::OutputDebugString(TEXT("*** Error - サイズ取得失敗(CSoundBuffer_GetSize)\n"));
		return 0;
	}
...
UINT CSoundBuffer::GetLengthMSec(){

	DSBCAPS dsbcaps;
	dsbcaps.dwSize = sizeof(dsbcaps);
	if (m_pDSBuffer->GetCaps(&dsbcaps) != S_OK){

		::OutputDebugString(TEXT("*** Error - 長さ取得失敗(CSoundBuffer_GetLengthMSec_dsbcaps)\n"));
		return 0;
	...
	if (m_pDSBuffer->GetFormat(&format, size, NULL) != S_OK){
		...
		::OutputDebugString(TEXT("*** Error - フォーマット取得失敗(CSoundBuffer_GetLengthMSec_format)\n"));
		return 0;
...
void CSoundBuffer::SetPositionMilliSec(int millisec){
	...
	if (m_pDSBuffer->GetFormat(&format, size, NULL) != S_OK){
		...
		::OutputDebugString(TEXT("*** Error - 長さ取得失敗(CSoundBuffer_SetPositionMilliSec_format)\n"));
		return;
