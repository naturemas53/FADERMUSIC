...
public:
	...
	// ロング整数
	inline void SetLongIntegerForKey(const char* pKey, const long    inValue) { m_MapLong[pKey] = inValue; }
	inline void RemoveLongIntegerForKey(const char* pKey)						{ m_MapLong.erase(pKey); }
	long GetLongIntegerForKey(const char* pKey);
...
private:
	...
	std::map<std::basic_string<char>, long   >   m_MapLong;
